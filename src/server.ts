...
export function createStudyServer(provider: StudyProvider): McpServer {
  ...
  server.tool("list_courses", "List the fictional courses available in the mock provider.", async () =>
    asText(await provider.listCourses()),
  );

  server.tool(
    "get_upcoming_work",
    "Get fictional upcoming work due within a requested number of days.",
  ...
  server.tool(
    "get_announcements",
    "Get fictional LEARN-style announcements for a course.",
  ...
  server.tool(
    "piazza_list_folders",
    "List fictional Piazza folders for a course.",
    { course_id: z.string().min(1) },
  ...
  server.tool(
    "piazza_search_posts",
    "Search fictional Piazza posts by course and text query.",
  ...
  server.tool(
    "piazza_get_post",
    "Get one fictional Piazza post by course and post identifier.",
